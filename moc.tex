...
%\newcommand\addb[1]{\textcolor{purple}{\textbf{#1}}}
%\newcommand\removeb[1]{\textcolor{purple}{\xout{#1}}}
%\newcommand\replaceb[2]{\removeb{#1} \addb{#2}}
\newcommand\addb[1]{#1}
\newcommand\removeb[1]{}
\newcommand\replaceb[2]{\addb{#2}}
...
\editor{\add{Francois-Xavier Pineau}}
...
\begin{abstract}
This document \replace{describes}{defines version 2.1 of} the Multi-Order Coverage map \add{(MOC)} method \remove{(MOC)}
\remove{version 2.0} \replace{to specify}{, which enables the specification of} arbitrary coverages \replace{for}{across} sky regions
\replace{and/or time coverages and potentially other dimensions}{, time intervals and frequency domains}.

The goal is to be able to
provide a very fast comparison mechanism between coverages. The
mechanism \replace{is based}{relies} on \replace{a discretization of}{discretizing} \replace{space}{the unit sphere} and \replace{time
dimensions}{one-dimensional quantities such as time or frequency}.
\add{Specifically, the discretization uses the Z-order curve for 2-dimensional sky regions and on hierarchical dyadic subdivisions for 1-dimensional quantities.}
...
collections. \remove{There are already a few applications and libraries which
are taking advantage of this major evolution of the MOC standard.} 
\add{This version 2.1 extends the use of MOCs to the three physical quantities: 
...
This document is a \replace{major}{minor} release of
the already existing encoding method recommendation \emph{Multi-Order
  Coverage} map \citep[MOC~\replace{1.1}{2.0},][]{2022ivoa.spec.0727F}.
We generalize the MOC
originally limited to \replace{s}{S}pace \add{, Time, and Space-Time} dimension\add{s}
 (Space MOC, SMOC\add{; Time MOC, TMOC; and Space-Time
MOC, STMOC}) to \replace{the time
dimension (Time MOC, TMOC), and space and time dimensions (Space-Time
MOC, STMOC)}{the Frequency, Space-Frequency and Frequency-Time dimensions
(Frequency MOC, FMOC; Space-Frequency, SFMOC; Frequency-Time, FTMOC)}.
Figure~\ref{fig:ivoadiagram} illustrates the role MOC\replace{2.0}{ 2.1}
...
and manipulate space \replace{and}{,} time \add{and frequency} coverage in such a way that basic
operations like union, intersection\replace{,}{or} equality test can be performed
...
exposed below.
\replace{The standard MOC1.0 was limited to space, but for a
multitude of use cases in astronomy we need the notion of time to be
properly integrated}{The evolution of the MOC standard -- which was initially
limited to spatial quantities in version 1.0, then expanded to include temporal quantities in version 2.0, 
and finally supplemented with frequency quantities in version 2.1 
-- enables it to cover a wide range of astronomical use cases}, e.~g.:
\begin{itemize}
...
\item \add{Is a radio source contaminating my observations in given frequency ranges?}
\end{itemize}
...
ipyaladin\footnote{\url{https://cds-astro.github.io/ipyaladin/}} widget\footnote{\url{https://pos.sissa.it/357/031/pdf}}. 

\remove{Adding temporal information in a space MOC encoding can provide a
...
\subsection{\replace{Multi-site positional and temporal search}{Multi-site search in space, time and/or frequency}}
...
and/or at a given time \add{, and/or at a given frequency}.
While this is in principle possible by
...
and time\add{/frequency} of interest would then be queried. This would provide a great
...
\subsection{\add{Low frequency radio astronomy}}

\add{Ground-based low-frequency radio astronomy instruments typically span over
...
\add{The low-frequency radio sources observed in our solar system,
...
\add{Irrespective of the quantity,} the MOC standard is defined using four basic building blocks:
...
\item Determine a proper tessellation/discretization methodology for
  each \replaceb{dimension axis}{quantity} (space, time, \add{frequency}, ...);
\item Fix a unique referential system for each \replaceb{dimension}{quantity}, to avoid 
...
\item Use an hierarchical procedure and a unique representation
  (canonical form) for compacting and quickly manipulating \removeb{each axis}
  coverage\addb{s of a given quantity} at any level of accuracy;
...
\replace{or}{, } temporal \add {or frequency} axis.
...
\remove{To work efficiently on existing hardware, we encode of any pair
...
%(order, index) as a \replace{long}{64-bits} integer \rm{(64 bits)}, and we reserve the two
%most significant bits to encode the type of MOC (spatial, temporal,
%\add{frequency} or \replace{future usages}{custom}). 
%The earlier MOC standard was limited to spatial\add{, time, and space-time} coverage\add{s}.
%We are reusing these principles to manipulate \replace{temporal}{frequency} coverages,
%as well as \replace{space-time}{space-frequency and frequency-time} coverages where we can manipulate the two
%physical dimensions simultaneously.

\medskip
\par\noindent
We will now explain the conventions chosen for \rm{the spatial, temporal and frequency axis}
\add{single quantity MOCs, namely space, time and frequency MOCs}
\replaceb{designed}{designated} \add{by the acronyms SMOC, TMOC and FMOC.
...
\replaceb{designed}{designated} \add{by the acronyms STMOC, SFMOC and FTMOC.}

\subsection{\add{Single quantity MOCs}}

\add{In this section, we describe the convention relative to single quantity MOCs.}
...
\item Standard: Existence of many HEALPix libraries: C++, Java,
  Fortran, \replace{IDL}{Python, Rust, Web Assembly, }...
  Also, HEALPix was selected for several all sky
  missions such as WMAP, Planck and Gaia. The HEALPix main web site \replaceb{is}{, previsously}
  located at Jet Propulsion Lab\addb{oratory}\addb{, is now on sourceforge} (\url{http://healpix.sourceforge.io/}).
\end{itemize}

The HEALPix \citep{2005ApJ...622..759G} \add{NESTED} tessellation technique divides
...
% MOVED HERE FROM v2.1-v2
\add{In practice, the sub-division of each base cell -- 
which are squares in the Euclidean HEALPix projection plane -- consists in 
iterations of a Z-order curve, and bit interleaving allows for
a non-recursive algorithm.}
\addb{When encoding a "NESTED" HEALPix index, the four Most Significant Bits (MSB) are used to code
...
% MOVED HERE FROM v2.1-v2
\replace{This}{Thus, the "NESTED"} numbering consists of enumerating all cells in a
...
\remove{HEALPix allows three coordinate systems: galactic, equatorial and
...
\replaceb{The mechanism used to determine which HEALPix cell contains a given
The formula computing a NESTED HEALPix index provided a given $order$
and}{The formula computing a cell index from a} sky location is described in
the main article defining the HEALPix system \citep{2005ApJ...622..759G}.
\replaceb{Several support libraries supporting the most important set of primitives are
already available.
...
basic SMOC arithmetics.}{Several support libraries implementing the most
...
JD=0.
\addb{Hence, ab integer time value $t$ in $\mu$s since JD=0 is valid if 
...
% MOVED HERE fom v2.1-v2 
\addb{We note that two consecutive cells at order N with the indices
...
\remove{The time is a relative observation, and depends on the position of the
...
\remove{Our choice is motivated by the fact that this system is linear by
construction and has been adopted by numerous missions such as Gaia.}

% MOVED HERE fom v2.1-v2 
TMOC {\bf must} be based on \removeb{JD system,} the time
scale TCB (see section~\ref{sec:tcb}) and the Solar System Barycenter as the reference
...
Table~\ref{table:tmocorders}
\replaceb{is showing some time values at a given order}{shows the time resolutions of the cells for each possible order}.
...
\subsubsection{\add{Frequency MOC conventions}\label{sec:fmoc1}}

\add{As for Time, representing a frequency coverage requires defining \emph{a priori}
...
\add{The adopted\footnote{To handle floating-point quantities, two approaches have been proposed.
...
\add{For a given $order$, the interval $[0,1[$ is subdivided into $2^{order+1}$ dyadic intervals (the MOC cells),
...
\add{The adopted bounds of the valid frequency range are:
\begin{itemize}
...
\add{The mapping function $g$ is defined as a linear interpolation in logarithmic space:
\begin{equation}
  g(f) = \frac{\log_{10}(f) - \log_{10}(f_{min})}{\log_{10}(f_{max}) - \log_{10}(f_{min})} 
       %= \frac{\ln(f) - \ln(f_{min})}{\ln(f_{max}) - \ln(f_{min})}
  \label{eq:fmoc:g}
\end{equation}}

\add{Combining Eq.~(\ref{eq:fmoc:i}) and Eq.~(\ref{eq:fmoc:g}) with the selected frequency range,
...
\begin{equation}
  ...
  \right\rfloor
  %= \left\lfloor 
  %\frac{\ln(f \times 10^{18})}{56 \ln(10)} \times 2^{order+1} 
  %\right\rfloor
\end{equation}}

\add{The number of consecutive integers that can be exactly represented by an IEEE~754 floating-point
is limited by the number of bits in the mantissa.
For double-precision floating-points, this number is 52.}

\add{Notably, for a fixed exponent, the fractional part of a floating-point number corresponds to
a dyadic rational. Consequently, there is no benefit in defining a MOC $order$ with more cells
than the number of dyadic rationals representable with 52 bits. Beyond this limit, the cells
would be smaller than the intrinsic precision of double-precision floating-point numbers.}

\add{For this reason, the maximum order is limited to 52 levels, ranging from 0 to
$order_{max} = 51$.
At the maximum resolution, the index corresponding to a frequency $f$ is therefore computed as:
\begin{align}
  order_{max} &= 51 \\
...
\add{Conversely, the frequency range $[f_{min}, f_{max}[$ corresponding to a given index $index$ at
a given order $order$ can be obtained using the inverse transformation:
\begin{eqnarray*}
...
\add{Choosing a given order is equivalent to selecting the number of mantissa bits used to represent the mapped value 
in the interval $\in [0, 1[$.
In practice, this corresponds to choosing a level of numerical precision, analogous to selecting a 
number of significant bits (or digits), noting that approximately one decimal digit of precision is
lost for every 3 to 4 mantissa bits removed.
The approximate number of significant decimal digits corresponding to a given order can be estimated using
...
\add{We can also express, for a given order $order$ the relative size of cell using $g^{-1}$:
\begin{equation}
...
\addb{Unlike time, the finest resolution is not constant across the full frequency range.
...
\addb{Like for time, two consecutive cells at order $N$, with indices
...
\addb{Considering the dyadic structure of the intervals, a cell at order
...
%\paragraph{\add{Remark}}
%
%\add{To handle floating-point quantities, two approaches have been proposed.
%The first implemented solution is described in Appendix~\ref{sec:apx:fmoc1}.
%It was later replaced by the more WCS-compliant solution presented above.}

\subsection{\add{Two dimensional MOCs}}

\add{In this section, we describe the convention adopted\footnote{The use of
...
First, the resolutions of both quantities would have been coupled, making it impossible, for example,
to describe an observational coverage with low temporal resolution and high spatial resolution.
Second, encoding two 64-bit indices into a single 64-bit index would have reduced the maximum
achievable resolution for each quantity.}
for two-dimensional MOCs,
i.e. MOCs describing the coverage of two quantities at the same time.}

\add{To respond to the different use cases presented at the beginning of 
the document, independent single quantity MOCs are not enough.
We need to link two quantities in a global mechanism.
For example, we need to be able to select a SMOC using a time or a frequency window.
...
\add{Our approach combines two quantities -- time and space, frequency and space, or time and frequency --
by associating sub-MOCs of the first quantity with MOCs of the second quantity.
...
In the various serializations, a 2D-MOC is represented as a set of tuples,
each consisting of a sub-MOC of the first quantity and a MOC of the second quantity.
Although the resolutions of the first and second quantities may differ, the resolution of each quantity remains uniform across the entire 2D-MOC.}

\addb{In the remaining of the document we adopt the following convention: 
a $YX$MOC is a 2D-MOC in which $X$ is the first quantity and $Y$ the second one.}

%\add{Remark: the use of a single index obtained by interleaving the indices of two quantities,
%similarly to a 2D Z-order curve, was considered but ultimately discarded.
%First, the resolutions of both quantities would have been coupled, making it impossible, for example,
%to describe an observational coverage with low temporal resolution and high spatial resolution.
%Second, encoding two 64-bit indices into a single 64-bit index would have reduced the maximum
...
\remove{To respond to the different use cases presented at the beginning of
...
\remove{Our approach is to combine these two dimensions - time and space - by
...
\remove{This two-dimensional interleaving approach has the advantage of making
...
\add{In Space-Time MOCs (STMOCs), the first quantity is time and the second quantity is space.
Consecutive time ranges associated with the same spatial coverage
are grouped into a sub-TMOC linked to their common SMOC.
...
in which at a given \replace{TMOC range}{sub-TMOC} we obtain the corresponding SMOC. 
...
\subsubsection{\add{Space and Frequency MOC conventions}}

\add{In some use cases, it is desirable to combine the spatial and frequency dimensions.
...
\addb{Thus, in Space-Frequency MOCs (SFMOCs), the first quantity is the frequency
and the second quantity is space.}
\add{The type of quantity is specified by a prefix in the ASCII representation or 
by a keyword in the FITS serialization.}

\subsubsection{\add{Frequency and Time MOC conventions}}

\add{FTMOCs are used to combine the frequency and time dimensions.
They use the same convention as STMOCs and SFMOCs, with the appropriate quantities prefix
in the ASCII serialization.}
\addb{Thus, in Frequency-Time MOCs, the first quantity is the time
and the second quantity is the Frequency.}


\section{\replace{SMOC and TMOC encoding}{MOC serializations}}

\removeb{The encoding described in this section guarantees backward compatibility
with MOCs corresponding to previous versions of this standard.}
\addb{Two complementary serialization formats are defined for all MOCs:
a string serialization based on ASCII and a binary format based on FITS.}

\subsection{\replace{SMOC and TMOC encoding}{MOC serializations for a single quantity}}

%\subsection{Space MOC \remove{or SMOC}}
%
%\remove{As introduced above, the SMOC {\bf should} be based on the
...
%the unique reference for each body will have to be defined.}
%\remiveb{Two complementary encoding formats are defined: a string serialization based
%on ASCII and a binary format based on FITS.}
...
%\add{In a "NESTED" HEALPix index, the four Most Significant Bits (MSB) are used to code the base cell index 
%($ \in [0, 12[)$ and the remaining LSBs are the 2-dimensional Morton code of the coordinates inside the base cell.
%Said differently, an HEALPix number comes from the concatenation of 12 Z-order curves, one curve per base cell.}
...
%\subsection{Time MOC \remove{or TMOC}}
%\remove{As introduced above, }TMOC {\bf must} be based on JD system, the time
%scale TCB (see section~\ref{sec:tcb}) and the Solar System Barycenter as the reference
%position \citep[see also][]{2019ASPC..523..497F} and section~\ref{sec:tmoc1}.
%The best resolution supported by TMOC is $1 \mu$s.
%
%\removeb{In the case that the time scale and the time reference position are unknown,
...
%\subsection{\add{Frequency MOC}}
%
%\add{As introduced in section~\ref{sec:fmoc1}, the quantity manipulated by a FMOC
%{\bf must} be a frequency, expressed in $Hz$.}
%
%\add{FMOCs supports 52 orders, conresponding to the number of explicitly stored
%mantissa bits in an IEEE 754 double-precision binary floating-point number.}
%
%\add{Unlike time, the finest resolution is not constant across the full frequency range.
%Because frequencies are represented using 64-bit floating-point values,
%the achievable precision is \textit{relative} rather than \textit{absolute}.
%At the highest resolution (order 51), the relative precision is approximately $10^{-15}$,
%which is close to the number of significant decimal digits provided by double-precision
%floating-point numbers.}
%
%\add{Two consecutive cells at order $N$, with indices
%$(M\times 2)$ and $(M\times 2)+1$, are represented at order $N-1$
%by a single cell with index $M$. 
%This hierarchical structure applies recursively across all orders.}
%
%\add{Considering the dyadic structure of the intervals, a cell at order 
%$N-1$ spans twice the frequency range of a cell at order $N$
%Consequently, the frequency resolution at order 
%$N-1$ is half that at order $N$.}
...
\add{While} \replace{T}{t}hese two methods are used for
SMOCs, \replace{TMOC and STMOC serializations and are presented
below}{only the intervals approach is used for other MOCs}.
\add{The various possible MOC} serializations are presented below.
...
\paragraph{\add{Remark on RANGE vs UNIQs packing}}

\add{For one-dimensional quantities such as time or frequency, the RANGE representation
...
RANGE packaging has been introduced for MOC\replace{2.0}{ 2.0}.
...
not yet compatible with MOC\replace{2.0}{ 2.0}.
...
The list of cells {\bf must} be well-formed\replaceb{,}{.}
\addb{Both the orders} and \removeb{the} values {\bf must}
be \addb{sorted} in ascending numerical order\addb{, comparing first the order and
then the cell value}.
\addb{This guarantees the same cell orderering as in the UNIQ FITS serialization,
...
times”; iii) postfix ? means "0 or 1 times". The first \replace{three}{six}
rules depend on the MOC type, i.e. SMOC for space, TMOC for
time, \add{FMOC for Frequency} \replace{and}{,} STMOC for space-time
\add{,SFMOC for space-frequency and FTMOC for frequency-time}. 


\subsection{\replace{STMOC encoding}{MOC serialization for quantity pairs}}
\label{sec:2dmoce}


%\removeb{\add{In this section, we first describe the encoding of STMOC.
%The same principles apply to the other possible combinations of quantities.}}

%\subsection{STMOC encoding}

Coding \replaceb{STMOC}{a 2D-MOC} consists in the following: 
for each \replaceb{element ofa temporal coverage}{sub-MOC of the first quantity}
we list the associated \replaceb{spatial coverage}{MOC of the second quantity}
using the natural packaging as defined in the previous section. 
...
\replaceb{time}{the first quantity (time)} 
and \replaceb{space}{the second quantity (space)} indices,
the \replaceb{time}{first quantity} indices {\bf must} have the 64th bits forced
to 1. It is not a sign inversion (two's complement) but a mask affecting
only that \replaceb{last}{most significant} bit
without \replaceb{touching}{affecting} any other bits.
The order of dimensions is always \replaceb{time}{the first quantity (time)} first.
...
%\subsection{\add{SFMOC and FTMOC encoding}}
%\label{sec:sfmoc}

%\add{The SFMOC and FTMOC serializations are the same as the STMOC serialization},
%\removeb{using the prefix 'f' for the Frequency quantity.}
%\addb{except that:
%\begin{itemize}
%  \item the first quantity of a SFMOC is the Frequency;
%  \item the second quantity of a FTMOC is the Frequancy;
%  \item the prefix 'f' is used for Frequency in the ASCII serialization. 
%\end{itemize}
%}

\subsubsection{ASCII Serialization}
The ASCII serialization of a \replaceb{ST}{2D-}MOC is a string
following the ASCII MOC serialization presented below, which interleaves
\replaceb{time coverage}{first quantity sub-MOCs} \removeb{as a
excerpt of TMOC} and associated
\replaceb{space coverage as a SMOC}{second quantity MOCs}.
\removeb{Each TMOC
element {\bf must} be prefixed by the character 't', and each SMOC
element {\bf must} be prefixed by the character 's'.}
\addb{Each sub-MOC and MOC {\bf must} be prefixed by the appropriate character:
...
\subsection{\add{Reference frames}}

\subsubsection{\add{SMOC and ICRS}}
...
\subsubsection{\add{TMOC and TCB} \label{sec:tcb}}
...
\subsubsection{\addb{TMOC of unknown time scale and reference}}
...
one unique representation for one coverage \add{, for both the possible UNIQ (SMOC) and RANGE alternatives}.
...
siblings for SMOC, 2 siblings for TMOC \add{and FMOC}). In the case of range
...
(see Appendix~\ref{app:perf}). In the case of STMOC \add{(or SFMOC)}, it is possible to adjust
the spatial order and/or on the temporal \add{(or frequency)} order independently.

\subsection{Working resolution}
The MOC has been \replaceb{designed}{designated} to be able to efficiently handle observation
...
We have reviewed the standards for encoding the different MOC flavors\replaceb{,}{:} 
\addb{in addition to} the space MOC \replaceb{(already described in version 1.1 of this document),
SMOC, the time MOC, TMOC and the space-time MOC, STMOC}{(SMOC), time MOC (TMOC)
...
\begin{itemize}
\item The SMOC is simply defined \addb{either} as a list of HEALPix indices (order, index) 
      \addb{or as a sorted list of non-overlapping indices ranges}.
...
\item The TMOC is \removeb{simply} a list of \addb{sorted and non-overlapping ranges of} time \removeb{interval} indices \replaceb{(order, index)}{at the deepest order}.
...
\item \addb{The FMOC is also a list of sorted and non-overlapping ranges of frequency indices at the deepest order}
\item \addb{The conversion from Frequency, in Hz, to deepest order frequency index is provided by Eq. \ref{eq:fmoc:i}}
\item \addb{Similarly to time, frequency index intervals are grouped 2 by 2 with a total of 52 orders.}
\end{itemize}

Once defined and encoded for a given astronomical collection, one
can easily combine the MOCs for \addb{two of} these \replaceb{two dimensions}{three quantities}
to create a merged \replaceb{STMOC}{2D-MOC} which can be used to navigate and access the collection
through their coverage for both \replaceb{time and space}{its quantities} simultaneously.
...
\subsection{\add{Changes between versions 2.0 and 2.1}}

\add{The differences between version 2.1 of MOC and the preceding version 2.0 are:
\begin{itemize}
...
\input{appendix_fmoc1.tex}
\newpage
\input{appendix_custom.tex}
\input{appendix_packing.tex}
