...
\author{Thomas Robitaille (Aperio Software)}
...
The goal is to be able to provide a very fast comparison mechanism between
coverages. The mechanism relies on discretizing the unit sphere and
one-dimensional quantities such as time or frequency.
Specifically, the discretization uses the Z-order curve for 2-dimensional sky
regions and hierarchical dyadic subdivisions for 1-dimensional quantities.
...
and manipulate space, time and frequency coverages in such a way that basic
...
quite a big hurdle for the researchers. With MOC 2.0 and 2.1 it is possible
...
each HEALPix order, while Figure~\ref{fig:smoc-view} shows a MOC creation,
from images to their coverage, with their corresponding HEALPix numbers.
...
JD=0.
Hence, an integer time value $t$ in $\mu$s since JD=0 is valid if
$t \in [0, 2^{62}[$. For a given $order$, its cell $index$ is:
\begin{equation}
  index = \frac{t}{2^{61-order}}
  \label{eq:tmoc:i}
\end{equation}
%  = 2^{order_max - order} (useful with a different datatype in which order max is different)
Conversly, the range of time values $[t_{min}, t_{max}[$, in $\mu$s since JD=0,
of a given $index$ of order $order$ is given by
\begin{align}
  t_{min} &= index \times 2^{61-order} \\
  t_{max} &= (index + 1) \times 2^{61-order}
  \label{eq:tmoc:t}
\end{align}.
...
See Table~\ref{table:tmocorders} for the list of time resolutions
of TMOC cells.
...
Thus, TMOC {\bf must} be based on the time scale TCB (see section~\ref{sec:tcb})
...
It was later replaced by the more WCS-compliant solution presented in the core of the document.}
approach is fully generic.
...
For the coding of the RANGE alternative packaging, all indices {\bf shall}
be expressed at the deepest order: 29 for SMOCs, 61 for TMOCs and 59 for FMOCs.
The sequence of intervals {\bf shall} be stored in the FITS table as pairs
of signed integers.
For orders strictly less than 14 (SMOC), 30 (TMOC) or 28 (FMOC),
32-bit signed integers (TFORM1='J') {\bf may} be used.
In such a case, all indices {\bf shall} be expressed at order 13 (SMOC), 29 (TMOC) or 27 (FMOC).
Otherwise, 64-bit signed integers (TFORM1='K') {\bf must} be used.
The two integers {\bf must} represent the smallest index value of the interval
(inclusive) and largest value of the interval (exclusive), respectively.
The RANGE values {\bf must} be stored in ascending numerical order.
The resulting list {\bf shall} be stored in a single-column binary table extension. 
