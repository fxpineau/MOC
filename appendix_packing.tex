\section{\add{SUNIQ and ZUNIQ packaging}}

\add{Although they are not used in this standard, the SUNIQ and ZUNIQ packing schemes 
are implemented in some MOC libraries and are therefore briefly described here.
They are valid with all quantities.}

\subsection{\add{SUNIQ packaging}}

\add{SUNIQ provides a generalized alternative to the UNIQ packing, which is specific to HEALPix indices.
...
\add{The inverse operation uses the {\textit leading\_zeros} function, 
...
\add{Sorting UNIQ or SUNIQ values produces the same ordering of cells: cells of lower order appear first, and within a given order, cells are ordered by increasing index.}


\subsection{\add{ZUNIQ packaging}}

\add{ZUNIQ encodes both the cell index and its order in a single integer while preserving the 
...
\add{The inverse transformation uses the {\textit trailing\_zeros} function, 
